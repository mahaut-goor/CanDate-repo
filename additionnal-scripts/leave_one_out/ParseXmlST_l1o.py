...
#            USAGE INFO            #
#######################################
# Script to update the xml file for leave-one-out analyses
# It replaces the sample id in the xml file by the new sample id (ie the one that is left out) and updates the alignment block with the new fasta file containing the new sample id
# It also updates the prior name in the xml file by the new prior name (ie the one that is left out)
# It also updates the fasta id in the xml file by the new fasta id (ie the one that is left out)

# Usage: python ParseXmlST.py <input.xml> <alignment.fasta> <output.xml> <new_sample_id>
...
def replace_taxon_ids(root, old_id, new_id):
    ...
    for elem in root.iter():
        ...
        if elem.tag == "trait" and elem.attrib.get("traitname") == "date-backward":
            continue
        for attr, attr_value in list(elem.attrib.items()):
            if old_id in attr_value:
            # print(f"Replacing in element: {elem.tag} attribute: {attr} value: {attr_value}")
                count += 1
                elem.attrib[attr] = attr_value.replace(old_id, new_id)

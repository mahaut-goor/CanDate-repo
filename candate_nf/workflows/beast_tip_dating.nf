...
workflow BEAST_TIP_DATING {
    ...
    if( params.skip_consensus == false && params.beast_resume == false ) {
    ...
    if(params.skip_maln == false && params.beast_resume == false){
    ...
    if(params.skip_parse_xml == false && params.beast_resume == false){
    ...
    if(params.beast_resume == true){

            if(!params.mcmc_resume_map){
                //
                ...
                ch_beast_rerun = PYTHON_GENERATE_MCMC_RESUME_CSV.out.mcmc
                                       .splitCsv(header: true, sep: ',')
                                        .map{row->tuple([id:row.sample],row.xml_file,row.chain,row.run,row.state,row.log,row.tree)}
